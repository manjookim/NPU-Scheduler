...
# -*- coding: utf-8 -*-
"""
build_xlsx_det_v8s_vs_v5npu.py — Det 단일모델 v8s(CPU 후처리) vs v5-nms_core(NPU 후처리)
FPS=60 실험 결과를 컬럼설명/전체/조건별_3회평균 3개 시트짜리 xlsx로 정리한다.
(results_singlemodel_fps5to25.xlsx와 동일한 시트 구성 관례 — 컬럼설명/전체(N행)/평균(N행))
...
COLUMN_DESC = [
    ("실험 조건",
     "Det(Detection) 단일모델만 대상. HEF 2종 비교: yolov8s.hef(engine=cpu, 후처리를 host CPU에서 "
     "수행) vs yolov5xs_wo_spp_nms_core.hef(engine=nn_core/auto, 후처리 상당 부분을 NPU neural "
     "core에서 수행). batch=1/threshold=1/timeout=0ms/priority=0/INPUT_FPS=60으로 두 조건 동일 "
     "고정. 모델 버전(v8s/v5)과 입력 크기(640/512)는 YOLOv8이 NPU 후처리를 지원하지 않아 "
     "불가피하게 다름(PROJECT_SUMMARY.md §6) — 그 외 조건은 모두 통일."),
    ("run_id", "같은 조건 안에서 몇 번째 반복 실행인지"),
    ("hef_name", "실행된 모델 라벨 — 'Detection-CPU-baseline'(yolov8s, CPU 후처리) 또는 "
                 "'YOLOv5-NPU-postprocess'(yolov5xs_wo_spp_nms_core, NPU 후처리)"),
    ("img_size", "모델 입력 한 변 크기(정사각) — v8s=640, v5xs_nms_core=512"),
    ("batch / threshold / timeout_ms / priority",
     "HailoRT 스케줄러 파라미터. 이 실험에서는 1/1/0/0으로 두 조건 동일 고정(모델 1개 단독 "
     "실행이라 스케줄링 경합 자체가 없음)"),
    ("frame_count", "정상적으로 latency가 측정된 프레임 수(데이터셋 전체, 보통 약 673장)"),
    ...
    ("total_time_s", "이 조건에서 데이터셋 전체(약 673장)를 처리하는 데 걸린 총 wall-time(초)"),
