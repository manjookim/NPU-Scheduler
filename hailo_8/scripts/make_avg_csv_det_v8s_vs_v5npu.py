...
# -*- coding: utf-8 -*-
"""
make_avg_csv_det_v8s_vs_v5npu.py — Det 단일모델 v8s(CPU 후처리) vs v5-nms_core(NPU 후처리)
FPS=60 실험의 3회 반복을 조건(hef_name)별로 평균.

infer_yolov5_hailo8.cpp::save_csv_single()이 만든 CSV 전용 스키마:
...
hailo_8L/scripts/make_avg_csv.py(3모델 프레임워크용, use_det/seg/pose로 그룹핑)와 같은 방식이되,
이 파일은 단일 모델 1행 CSV라 hef_name 하나로 그룹핑한다는 점만 다르다.
