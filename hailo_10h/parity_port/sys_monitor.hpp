#pragma once
// sys_monitor.hpp — CPU/메모리/컨텍스트 스위치 측정 유틸 (Linux /proc 기반)
// [8L 대비 변경점] 없음. hailo_8L/sys_monitor.hpp와 바이트 단위로 동일한 로직을 유지한다.
// 두 보드의 cpu_percent/mem_percent/ctx_switches가 같은 방식으로 측정되어야 비교가 성립한다.
