...
// writer/reader 스레드 쌍(producer/consumer)으로 비동기 추론을 수행한다 (Hailo-8L).
// [의존] infer_scheduler.cpp의 파라미터 #define 블록(INPUT_FPS, ENABLE_POSTPROCESS,
...
// 와 timeout(대기 시간)이 실제로 트리거될 조건이 생긴다.
...
#if DEBUG_WRITE_TIMING
            // [진단용, 2026-07-28] 큐가 batch_size에 따라 커지는지 확인하기 위한 임시 계측.
            // write()가 즉시 반환되면(큐에 빈 슬롯 있음) 이 값은 ~0ms, 큐가 꽉 차서 블로킹되면
            // NPU가 한 프레임을 처리할 때까지 기다리는 시간만큼 값이 커진다. 몇 번째 프레임부터
            // 이 값이 튀는지 보면 host측에서 체감하는 실질 큐 깊이를 알 수 있다.
            if (i < 40) {
                double w_t1 = now_ms();
                std::lock_guard<std::mutex> lock(print_mutex);
...
#if DEBUG_WRITE_TIMING
    // [진단용, 2026-07-28] HailoRT 공식 큐 크기 accumulator 값 출력 (element별 min/mean/max).
    {
        std::lock_guard<std::mutex> lock(print_mutex);
