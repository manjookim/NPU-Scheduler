...
# -*- coding: utf-8 -*-
"""
fill_hrtt_columns_det_v8s_vs_v5npu.py
results_det_v8s_vs_v5npu_fpssweep.csv 의 HRTT 전용 NaN 컬럼(switches_per_s,
...
파일명 규칙: det_{v5npu|v8scpu}_fps{0|30}_run{1..3}_{timestamp}.hrtt
CSV 매칭 키: (hef_name, input_fps) 그룹 내에서 run_id 오름차순 = hrtt run{n} 오름차순
             (run_det_v8s_vs_v5npu_fpssweep.sh가 그 순서로 만들었으므로 순서 매칭 가능)

사용법: python fill_hrtt_columns_det_v8s_vs_v5npu.py <csv_path> <hrtt_dir> [출력_csv_path]
...
TAG_TO_HEF_NAME = {
    "v5npu":  "YOLOv5-NPU-postprocess",
    "v8scpu": "Detection-CPU-baseline",
}

FNAME_RE = re.compile(r"det_(v5npu|v8scpu)_fps(\d+)_run(\d+)_")
