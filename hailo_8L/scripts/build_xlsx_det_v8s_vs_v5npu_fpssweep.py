...
COLUMN_DESC = [
    ("실험 조건",
     ...
     "vstream이 이미 'HAILO NMS BY CLASS' 포맷으로 나옴을 실측 확인함). batch=1/threshold=1/"
     "timeout=0ms/priority=0으로 고정, INPUT_FPS만 {0(제한없음/최대속도), 30}로 스윕. 모델 "
     "버전(v8s/v5)과 입력 크기(640/512)는 YOLOv8이 NPU 후처리를 지원하지 않아 불가피하게 "
     "다름(PROJECT_SUMMARY.md §6) — 그 외 조건은 모두 통일."),
    ("run_id", "전체 실행 순번(1~12) — FPS=0(1~6) → FPS=30(7~12) 순서, 각 구간 내 v5-NPU 3회 → v8s-CPU 3회"),
    ...
    ("avg_postprocess_ms",
     ...
     "CPU 조건(Detection-CPU-baseline)은 host에서 NMS 연산까지 전부 수행한 시간이다 — 두 "
     "조건의 이 컬럼 값을 곧이곧대로 비교하면 'NPU가 압도적으로 빠르다'는 착시가 생길 수 "
