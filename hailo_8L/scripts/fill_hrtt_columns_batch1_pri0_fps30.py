...
# -*- coding: utf-8 -*-
"""
fill_hrtt_columns_batch1_pri0_fps30.py
2026-08-04_batch1_pri0_fps30_exp1 실험 전용.
traces/*.hrtt를 직접 파싱해서 csv/results_batch1_pri0_fps30.csv의 HRTT 전용
...
파일명 규칙: <ud>D-<us>S-<up>P_batch1_pri0_fps30_run<n>_<시각>.hrtt
사용법: python3 fill_hrtt_columns_batch1_pri0_fps30.py [실험폴더]
       (기본값: hailo_8L/experiments/2026-08-04_batch1_pri0_fps30_exp1)
"""
...
EXP_DIR = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    BASE, "experiments", "2026-08-04_batch1_pri0_fps30_exp1")
TRACES_DIR = os.path.join(EXP_DIR, "traces")
CSV_PATH = os.path.join(EXP_DIR, "csv", "results_batch1_pri0_fps30.csv")

FNAME_RE = re.compile(r"(\d)D-(\d)S-(\d)P_batch1_pri0_fps30_run(\d+)_")
