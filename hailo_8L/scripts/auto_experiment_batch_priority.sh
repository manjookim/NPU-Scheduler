...
#   4) threshold / timeout 은 이번 실험 대상이 아니므로 고정 (threshold=1[기본값], timeout=0)
#      * threshold=0은 우선순위 다른 다중모델에서 hang을 유발해 기본값 1 사용
#
...
#
# 주의:
#   - threshold=1 (HailoRT 기본값, 제외 대상). threshold=0은 우선순위 다른 다중모델에서
#     높은 우선순위 모델이 NPU를 안 놓아 낮은 모델이 hang됨 → 반드시 1 사용.
...
compile_and_run() {
    ...
    # threshold: 제외 대상 → 1 고정 (HailoRT 기본값)
    # 주의: threshold=0으로 두면 우선순위가 다른 다중모델에서 높은 우선순위 모델이
    #       큐가 비어도 계속 실행자격을 유지해 낮은 모델을 영구 starvation → hang 발생.
    #       기본값 1이면 큐가 비면 자격을 내려놔 정상적으로 스케줄이 넘어간다.
    sed -i "s/^#define THRESHOLD_DET .*/#define THRESHOLD_DET   1/"    infer_scheduler.cpp
    sed -i "s/^#define THRESHOLD_SEG .*/#define THRESHOLD_SEG   1/"    infer_scheduler.cpp
    sed -i "s/^#define THRESHOLD_POSE .*/#define THRESHOLD_POSE  1/"   infer_scheduler.cpp
    # timeout: 제외 대상 → 0 고정
