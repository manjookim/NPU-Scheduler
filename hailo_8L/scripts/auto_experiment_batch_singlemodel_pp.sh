#!/bin/bash
# =============================================================================
# [2026-07-29] 배치사이즈 스윕(싱글모델) x 후처리 ON/OFF 비교 실험 (Hailo-8L)
#
# 요청 내용:
...
#
# 실행 위치: 보드(rpi1, ~/hailo_cpp_test/), infer_scheduler.cpp / postprocess_8l.hpp와 같은 디렉토리.
...
if [ ! -f postprocess_8l.hpp ]; then echo "[오류] postprocess_8l.hpp 없음"; exit 1; fi
cp "$SRC" "${SRC}.bak"
echo "[백업] ${SRC} -> ${SRC}.bak"
