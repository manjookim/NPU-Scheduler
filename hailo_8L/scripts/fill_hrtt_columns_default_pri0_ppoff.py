...
# -*- coding: utf-8 -*-
"""
fill_hrtt_columns_default_pri0_ppoff.py
<날짜>_default_pri0_ppoff_exp1 실험 전용 (batch=1/threshold=1/timeout=0/priority=0,
ENABLE_POSTPROCESS=0 즉 후처리 시간 미측정).
traces/*.hrtt를 직접 파싱해서 csv/results_default_pri0_ppoff.csv의 HRTT 전용
...
파일명 규칙: <ud>D-<us>S-<up>P_pri0_ppoff_run<n>_<시각>.hrtt
사용법: python3 fill_hrtt_columns_default_pri0_ppoff.py <실험폴더>
       (예: hailo_8L/experiments/2026-08-06_default_pri0_ppoff_exp1)
"""
...
if len(sys.argv) > 1:
    ...
    # 실험 폴더명은 실행 날짜에 따라 달라지므로 인자 없이 실행 시 자동 탐색(가장 최근 것).
    candidates = sorted(glob.glob(os.path.join(BASE, "experiments", "*_default_pri0_ppoff_exp*")))
    if not candidates:
        sys.exit("실험 폴더를 찾지 못함 — 경로를 인자로 직접 넘길 것.")
    EXP_DIR = candidates[-1]

TRACES_DIR = os.path.join(EXP_DIR, "traces")
CSV_PATH = os.path.join(EXP_DIR, "csv", "results_default_pri0_ppoff.csv")

FNAME_RE = re.compile(r"(\d)D-(\d)S-(\d)P_pri0_ppoff_run(\d+)_")
