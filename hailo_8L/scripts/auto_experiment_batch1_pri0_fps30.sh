#!/bin/bash
# =============================================================================
# [2026-08-05] batch=1, priority=0, timeout=0, threshold=1 고정 조건 실험.
# auto_experiment_default_workload_fps30.sh(=SDK 진짜 기본값 batch=0/priority=16 조건)의
# 변형 — 이번엔 사용자가 명시적으로 지정한 값(batch=1, priority=0)으로 고정하고,
# 나머지(threshold=1, timeout=0)는 기존과 동일, 입력은 INPUT_FPS=30(카메라 실입력 속도 모사).
#
# 고정 파라미터:
#   - batch     = 1
#   - threshold = 1
...
# 사용법: chmod +x auto_experiment_batch1_pri0_fps30.sh
#         nohup bash auto_experiment_batch1_pri0_fps30.sh > batch1_pri0_fps30_log.txt 2>&1 &
#         disown
# =============================================================================
...
EXP_DATE=$(date +%Y-%m-%d)
EXP_NUM=1
EXP_DIR="experiments/${EXP_DATE}_batch1_pri0_fps30_exp${EXP_NUM}"
while [ -d "$EXP_DIR" ]; do
    EXP_NUM=$((EXP_NUM + 1)); EXP_DIR="experiments/${EXP_DATE}_batch1_pri0_fps30_exp${EXP_NUM}"
done
OUTDIR="$HOME/hailo_cpp_test/${EXP_DIR}"
CSV_DIR="$OUTDIR/csv"
TRACES_DIR="$OUTDIR/traces"
mkdir -p "$CSV_DIR" "$TRACES_DIR"
TARGET_CSV="$CSV_DIR/results_batch1_pri0_fps30.csv"

echo "실험 폴더 : $EXP_DIR"
echo "설정      : batch=$BATCH, threshold=$THRESHOLD, timeout=${TIMEOUT_MS}ms, priority=$PRIORITY, INPUT_FPS=$INPUT_FPS"
...
PY
    ...
    if [ -n "$LATEST_HRTT" ]; then
        ...
        NEW="${TRACES_DIR}/${USE_D}D-${USE_S}S-${USE_P}P_batch1_pri0_fps30_run${RUN_ID}_${TS}.hrtt"
...
echo "===== 완료! 7개 조건 × ${REPEAT}회 = $((7 * REPEAT))회 (batch=1, priority=0, timeout=0, threshold=1, INPUT_FPS=30) ====="
echo "CSV: $TARGET_CSV"
