...
// 그리고 vstream 생성까지 담당 (Hailo-8L). main()에서 순서대로 호출되는 두 함수로 구성.
// [의존] DEBUG_WRITE_TIMING 매크로를 사용하므로, infer_scheduler.cpp의 파라미터 #define
// 블록보다 뒤에 include 되어야 한다. classify_outputs()는 output_classify.hpp에서 온다.
    ...
    std::vector<std::vector<OutMeta>>& out_meta_per_ng)
{
    ...
    for (size_t k = 0; k < network_groups.size(); k++) {
        ...
        // 공식 제공하는 큐 크기 accumulator로 직접 측정하기 위해 파이프라인 통계 플래그를 켠다.
        // (하일로 커뮤니티에 batch_size에 따라 큐도 커진다는 언급이 있어, 실측으로 확인 중)
